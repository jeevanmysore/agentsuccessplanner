...
public class MonthGoalsController extends HttpServlet {
			...
			agent=session.getAttribute("userAgent").toString();
				...
				Calendar cal=Calendar.getInstance();
				 ...
				 if(goalsDto!=null){	
					 ...
					 request.setAttribute("monthGoals",goalsDto);
					 
					 if(agent.equalsIgnoreCase("web")){
					
						 RequestDispatcher rd=request.getRequestDispatcher("/pages/monthlyGoals.jsp");
						 rd.forward(request, response);
					 }
					 else{
						 RequestDispatcher rd=request.getRequestDispatcher("/mobile/monthlyGoals.jsp");
						 rd.forward(request, response);
					 }
				 }
